/***************************************************************
 *
 * DTNnode - Modificato da DAmendola - 7/3/2013 - Nodo DTN REQUESTER con Carrier Detection
 * + Ottimizzata usando function
 *
 * MODIFIED per ND test: RequesterCD, manda una NDReq e registra i vicini trovati.
 *
...
void sendBackNDRes (uint16_t s);
void NDResAlgorithm (void);
		...
		/****** BEGIN NDReq send & listen phase ******/
			...
			if ((nRFAPI_CarrierDetect () != 0x01))
			{
				...
				do{
					...
					// to see if there are collision...
					//if ((nRFAPI_CarrierDetect () != 0x01)) // canale free
						//logDataStorage(9999, LPC_TMR32B0->TC, ntohl(dtnMsg.NDres.seq), 6666, 99);
					//if ((nRFAPI_CarrierDetect ())) //canale busy
						//logDataStorage(9999, LPC_TMR32B0->TC, ntohl(dtnMsg.NDres.seq), 6666, 90);

					//logDataStorage(9999, LPC_TMR32B0->TC, ntohl(dtnMsg.NDres.seq), 6666, N);
					/**** if there is incomming packet recieve it *******/
					if (nRFCMD_IRQ ())
					{
						logDataStorage(9999, LPC_TMR32B0->TC, ntohl(dtnMsg.NDres.seq), 6666, 91);
						do // Start CONTENTION WINDOW
						{
							added = 0;
							//Riceve il messaggio
							nRFCMD_RegReadBuf (RD_RX_PLOAD, dtnMsg.byte,sizeof (dtnMsg));
					...
					else if ((nRFAPI_CarrierDetect ())) //canale busy
						logDataStorage(9999, LPC_TMR32B0->TC, ntohl(dtnMsg.NDres.seq), 6666, 99);
