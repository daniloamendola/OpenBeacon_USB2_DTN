...
float power (float b, uint32_t exp);
...
void NDResAlgorithm (uint32_t seq) {
	...
	while(s<300) // inizio finestra di Res
	{
		...
		{
			//Pr_sift = power((float)s,3);//?????????????????
			//=(((1-B10)*(B10^$F$6))/(1-((B10)^$F$6)))*(B10^(-$E10))
			uint32_t rnd_m = rnd(100);
			if (rnd_m<=pT) // mod DanAme 20 testbed con 10 e 40
			{ // Qui il valore P del pPersistent
				logDataStorage(s, rnd_m, 0x2220, 6666, pT);
				sendBackNDRes(s, seq);
				//break;
				pmu_sleep_ms (8);
				s=s+10;// al posto di break;
				if(s>200) pT=0;
			...
			{
				logDataStorage(s, rnd_m, 0x1110, 6666, pT);
				pmu_sleep_ms (8);
				s=s+10;
			}
		}//else
