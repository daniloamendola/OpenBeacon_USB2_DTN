/***************************************************************
 *
 * Modificato da DAmendola - 6/3/13 - DTN SENDER
...
main (void)
{
	...
	while (1)
	{
			...
			//if(onemsg<1)
		{
			...
			nRFAPI_SetRxMode (1); // 1 to receive: 0 to transmit
			nRFCMD_CE (1);
			pmu_sleep_ms (2); // for randomize the sending of msg
			nRFCMD_CE (0);
			...
			if ((nRFAPI_CarrierDetect () != 0x01)) // se il canale non è libero?
			{
				...
				//estratto il messaggio dalla coda lo metto nel Envelope e lo spedisco;
				dtnMsg.msg=msg;
				dtnMsg.proto=msg.proto;
				dtnMsg.msg.crc = htons (crc16(dtnMsg.byte, sizeof (dtnMsg) - sizeof (dtnMsg.msg.crc)));

				nRFAPI_SetRxMode(0);
				//	nRFCMD_CmdExec (W_TX_PAYLOAD_NOACK);
				nRF_tx (3);  // Sending masg. Il parametro indica la potenza ammessi 0,1,2,3.
				blink(1);
