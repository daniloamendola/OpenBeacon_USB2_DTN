/***************************************************************
 *
 * Modificato da DAmendola - 6/3/13
...
main (void)
{
	...
	while (1)
	{
		...
		/*//commento per realizzare sniffer

		// DTNMsg generation. Qui time in second
...
//delete begin
		...
		{ // check wherther it is a NDReq or not
			...
			{ // Se un DtnMessageEnvelope è arrivato correttamente

				/*
				if(!Contains(Q,dtnMsg.NDreq.seq) ) // Se il messaggio arrivato è già nella coda Q
				{
				...
				else // already has the msg whit this seq numer, so sleep and don't disrupt other nodes /
					pmu_sleep_ms (500);
				*/
			}
			nRFCMD_CE (0);
...
//delete end
			...
			if ((nRFAPI_CarrierDetect () != 0x01)) // se il canale non è libero?
			{
				...
				dtnMsg.proto = RFBPROTO_ND_REQ; //crea un messaggio di tipo NDreq
				dtnMsg.NDreq.from = htons (tag_id);
				dtnMsg.NDreq.time= htonl (LPC_TMR32B0->TC);
				dtnMsg.NDreq.seq = msgp->seq;  // msgp dove lo setta?
