...
main (void)
{
	...
	while (1)
	{
		...
		if(LPC_TMR32B0->TC - time >= 30)
			//if(onemsg<1)
		{
			//Generazione nuovo DTN_MSG
			msg.from = htons (tag_id);
			msg.proto = RFBPROTO_DTN_MSG;
			msg.prop = 1;
			msg.time= htonl (LPC_TMR32B0->TC);
			msg.seq = htonl(((0x00000000 | tag_id)<<16) | ++MsgSeq); //Msg Id is tagId:MsgSeq     //0x000 serve a trasformare in HEX
			Enqueue(msg, Q);
			time = LPC_TMR32B0->TC;
			onemsg++;

			//Faccio il log del messaggio appena creato
			logDataStorage(ntohl(msg.time), ntohl(0), ntohl(msg.seq), ntohl(0), msg.prop);
		...
		/***************************** END *****************************************/
			...
			if ((nRFAPI_CarrierDetect () != 0x01))
			{
				...
				if(N) // n!=0 // if find any neighbourd
				{
					...
					// Controllo se il Sink è tra i vicini.
					uint8_t isSink = 0;
					for(t=0;t<N;t++)
