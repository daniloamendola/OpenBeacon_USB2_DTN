...
storage_logtxt_fmt (char* buffer, uint32_t index)
{
  ...
  if( index < g_log_items )
  {
    ...
    if(crc8((uint8_t*)&msg, sizeof(msg)-sizeof(msg.crc)) == msg.crc)
    {
    	...
    	//uint8_t dim = sizeof("R%04X,%07u,%01c,%07u,%03u\n");
    	//msg.seq
    if(msg.from==6666)  // if: added by DanAme
  	  	  cIO_snprintf(buffer, LOGTXT_ENTRY_SIZE, "L%04X,%07u,%08X,%07u,%03u\n",g_device_id,msg.time2,msg.seq,msg.time1,prop);//msg.seq
    else if(msg.from>9999)  // Mod. by DanAme
    	  	  cIO_snprintf(buffer, LOGTXT_ENTRY_SIZE, "R%04X,%07u,%08X,%07u,%03u\n",g_device_id,msg.time2,msg.seq,msg.time1,prop);//msg.seq
      else
    	  //cIO_snprintf(buffer, LOGTXT_ENTRY_SIZE, "%02X\n", 0x1);
    	  	  cIO_snprintf(buffer, LOGTXT_ENTRY_SIZE, "S%04X,%07u,%08X,%07u,%03u\n",g_device_id,msg.time2,msg.seq,msg.time1,prop);
    }
    else
    {
