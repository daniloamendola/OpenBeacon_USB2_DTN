/***************************************************************
 *
 * DTNnode SIFT - Modificato da DAmendola - 10/04/2013 - ND_DTN_Responser con SIFT
 ...
 */
//static uint32_t
//siftRnd(uint32_t range)
//{
//	// che deve prendere in input??
//	// modificare per ritornare la distribuzione Sift
//	return range;
//
//}
...
void NDResAlgorithm (uint32_t seq) {
	...
	float alfa=0.8;
	uint32_t sift = 0;
	float constA = ((1.0-alfa)*pow(alfa, CW))/(1-pow(alfa, CW));
	//uint32_t siftSlot = 0;
	...
	sift = -log10((-log(alfa))*randU/constA)/log10(alfa);

	/* TEST CODE BEGIN */
	uint8_t i = 0;
	while(i<100) // inizio finestra di Res
	{
		float randU = ((float)rnd(100))/100;
		sift = -log10((-log(alfa))*randU/constA)/log10(alfa);
		logDataStorage(6666, sift, (randU*100), 6666, alfa*10); // with 6666 use a different log
		i++;
	}
	/* TEST CODE END*/
	while(s<300) // inizio finestra di Res
	{
		...
		else
		{
			...
			{ // Qui il valore P del pPersistent
				logDataStorage(2222, sift, rnd_m, 9999, 0);
				sendBackNDRes(s, seq);
				break;
			}
			else
			{
				logDataStorage(1111, sift , rnd_m, 19999, 0);
				pmu_sleep_ms (8);
				s=s+10;
			}
		}//else
...
//					//NDResAlgorithm(ntohl(dtnMsg.NDreq.seq));
					...
					/****** ********************** */
						...
						/* TEST CODE BEGIN */
						uint16_t i = 0;
						while(i<1100) // inizio finestra di Res
						{
							float randU = ((float)rnd(100))/100;
