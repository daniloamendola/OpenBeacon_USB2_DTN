 ...
 * + Select First Slot: sceglie casualmente tra i primi 10 slot da quale inizare a trasmettere
 *
 ...
 *	Modifichiamo per averer la selezione dello slot iniziale in cui tentare la Tx
 */
void NDResAlgorithm (uint32_t seq) {
	...
	while(s<300) // inizio finestra di Res
	{
		...
		else
		{
			...
			{ // Qui il valore P del pPersistent
				//logDataStorage(s, 2222, rnd_m, 9999, 0);
				sendBackNDRes(s, seq);
				break;
			}
			else
